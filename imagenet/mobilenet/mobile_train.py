...
from tensorflow.keras import optimizers, preprocessing, datasets, Model
import numpy as np
import os
...
def remove_black_borders(image):
    ...
    return np.min(y_nonzero), np.min(x_nonzero), np.max(y_nonzero), np.max(x_nonzero)
...
#     for j in range(32):
#         # np.append(box, remove_black_borders(img[j]))
#         box.append(remove_black_borders(img[j]))
#     # new_img = remove_black_borders(img[i])
# # box = np.asarray(box)
# # box = np.reshape(box, [-1, 32, 4])
# print(len(box),len(box[0]))
# print(box)
...
def main():
    ...
    optimizer2 = optimizers.Nadam(learning_rate=0.00002)
    ...
    # load data
    model.load_weights('./checkpoint/5.ckpt')
    INIT = 5
    EPOCH = 100
    ...
    for epoch in range(INIT, EPOCH):
        ...
        for step, (x, y) in enumerate(data):
            ...
            if step % 100 == 0:
                ...
                with summary_writer.as_default():
                    tf.summary.scalar('train-loss', float(loss), step=epoch * 300 + step)
        ...
        with summary_writer.as_default():
            tf.summary.scalar('acc', float(acc), step=epoch * 300 + step)
            tf.summary.image("val", val_images, max_outputs=25, step=epoch * 300 + step)
